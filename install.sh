...
if [ "$1" != "un" ]; then
	...
	if [ ! -f /etc/apt/sources.list.d/ookla_speedtest-cli.list ]; then
		...
		if [ -e /etc/os-release ]; then
			...
			echo "os=$os dist=$dist 1"
			os=$os dist=$dist curl -sSLN https://packagecloud.io/install/repositories/ookla/speedtest-cli/script.deb.sh | sudo bash
		else
	...
	cd /var/www/html
	rm -rf new_admin
	git clone --depth=1 https://github.com/ipitio/AdminLTE new_admin
	cd new_admin
	git fetch --tags -q
	...
	cd /opt/
	rm -rf new_pihole
	git clone --depth=1 -b ipitio https://github.com/ipitio/pi-hole new_pihole
	cd new_pihole
	git fetch --tags -q
	latestTag=$(git describe --tags `git rev-list --tags --max-count=1`)
	#git checkout $latestTag
